...
#         self.passengers_init = self._get_best_passengers(max_passengers=TaxiAgent.MAX_PASSENGERS)
#
#         self.set_all_possible_states()
#         self.set_all_possible_actions_and_next_states()
#         self.set_rewards_for_actions()
...
#         state_str = dict_to_str(state)
#
#         if state_str not in self.Best_Actions[t].keys():
#             # ~~~~~~ if state is unknown:
#             print(f"  {t=}", "unknown state")
#             # baseline
#             # legal_actions = self.actions(state_str)
#             # best_action = self.get_max_rewards_actions(legal_actions, top_num=1)[0]
#
#             # continue the tree
#             self.set_all_possible_actions_and_next_states(states_list=[state_str])
#             self.set_rewards_for_actions(list_actions=self.all_possible_actions_for_state[state_str])
#             self.value_iteration_with_dicts()
#             self.value_iteration_with_dicts(calc_depth=t, states_list=[state_str])
#
#         best_action = self.Best_Actions[t][state_str]
#         print(f"{t=}", best_action)
#         return best_action
#
#     def _get_best_pass_dict(self, state):
#         best_pass_dict = dict()
#         for pass_name, pass_dict in state['passengers'].items():
#             if pass_name in self.best_passengers_names:
#                 best_pass_dict[pass_name] = pass_dict
#         return best_pass_dict
#
#     # ~~~~~~~~~~~~~~~~~~~~~ ALL POSSIBLE STATES
#
#     def set_all_possible_states(self, state=None, prob_to_add_state=PROB_TO_ADD_STATE):
#         start = time.time()
#         """
#         Get all possible permutations of states
#         Retruns list of states as json strings.
...
#                 # checl legal capacity and passenger location
#                 if self.is_legal_capacity_and_passengers_locations(state):
#                     if random.random() < prob_to_add_state:
#                         # skip state
#                         continue
#                     self.all_possible_states.append(dict_to_str(state))
#
#         # print("\tset_all_possible_states finished after", time.time() - start)
#
#     def _get_best_passengers(self, max_passengers=2):
#         passengers_init = self.initial["passengers"]
#
#         # if (max_taxis is not None) and (len(taxis_init) > max_taxis):
#         #     # get the taxis with max fuel:
#         #     taxis_init_sorted = list(taxis_init.items())
#         #     taxis_init_sorted.sort(key=lambda x: x[1]['fuel'], reverse=True)
#         #     best_taxis = taxis_init_sorted[:max_taxis]
#         #     taxis_init = {x: y for x, y in best_taxis}
#
#         if (max_passengers is not None) and (len(passengers_init) > max_passengers):
#             # get the passengers with minimum possible_goals:
#             passengers_init_sorted = list(passengers_init.items())
#             passengers_init_sorted.sort(key=lambda x: len(x[1]['possible_goals']))
#             best_passengers = passengers_init_sorted[:max_passengers]
#             passengers_init = {x: y for x, y in best_passengers}
#
#         self.best_passengers_names = list(passengers_init.keys())
#         return passengers_init
#
#     def is_legal_capacity_and_passengers_locations(self, state):
#         """
#         example: num_passengers in taxi   = 3
...
#             all_options_by_passenger[passenger_name] = []
#             possible_locations = all_legal_locations_by_passenger[passenger_name]
#             possible_destinations = [passenger_details["destination"]]
#             # possible_goals and prob_change_goal do not change between states
#             possible_goals = passenger_details["possible_goals"]
#             prob_change_goal = passenger_details["prob_change_goal"]
#             for location in possible_locations:
...
#
#     def set_all_possible_actions_and_next_states(self, states_list=None):
#         """
#         Update:
#         @ list: self.all_possible_actions_for_state
#         @ list: self.all_possible_actions
...
#             states_list = self.all_possible_states
#         for state in states_list:
#             legal_actions = self.actions(state)
#             legal_actions = self.get_max_rewards_actions(legal_actions, top_num=TaxiAgent.NUM_TOP_ACT)
#             self.all_possible_actions_for_state[state] = legal_actions
#             if len(legal_actions) == 0:
#                 continue
#             if state == END_OF_GAME_STATE:
...
#                 for next_state, prob in next_states_with_probs.items():
#                     self.Next_States_Probs[(state, action)].append((next_state, prob))
#         # print("\tset_all_possible_actions_and_next_states finished after", time.time() - start)
#
...
#
#     def result_with_probs(self, state, action, prob_skip_calculations=PROB_SKIP_CALCULATIONS):
#         """
#         input: state, action
#         output: next possible states and the probability for each one
#         method:
...
#             return possible_next_state_probs
#
#         if random.random() < prob_skip_calculations:
#             prob = 1
#             possible_next_state_probs[result_state_str] = prob
#             return possible_next_state_probs
#
#         result_state = str_to_dict(result_state_str)
#         all_passengers_subsets = self.get_all_passengers_subsets(result_state)
...
#     # ~~~~~~~~~~~~~~~~~~~~~ VI
#
#     def value_iteration_with_dicts(self, calc_depth=None, states_list=None):
#         """
#         Get the best action for each step t
#         from t = 0 to t = max_t.
#         Update self.Best_Actions dict.
...
#         """
#         start = time.time()
#         if calc_depth is None:
#             calc_depth = self.max_turns_to_go
#
#         if states_list is None:
#             states_list = self.all_possible_states
#
#         for t in range(calc_depth + 1):
#             self.Values[t] = {}
#             self.Actions_Values[t] = {}
#             self.Best_Actions[t] = {}
#             if t == 0:
...
#                     # Update - Actions_Values dict
#                     for action in self.all_possible_actions_for_state[state]:
#                         # V(t, s) = max_over_a { sum_over_next_s[prob(next_s, a) * V(t-1, next_s)] + Reward(a) }
#                         value_of_action = self.Rewards[action]  # init
#                         for next_state, prob in self.Next_States_Probs[(state, action)]:
#                             try:
#                                 value_of_action += prob * self.Values[t - 1][next_state]
...
#                     self.Values[t][state] = best_value
#                     self.Best_Actions[t][state] = best_action
#         # print("value_iteration_with_dicts finished after", time.time() - start)
#
#     # ~~~~~~~~~~~~~~~~~~~~~
#
#     def get_max_fuel_per_taxi(self, initial):
